...
from .serializers import PostBaseInfoSerializer, PostDetailInfoSerializer
...
class PostBaseInfoListViewset(viewsets.ReadOnlyModelViewSet):
    ...
    List:
        文章列表页
    """
    queryset = PostInfo.objects.filter(is_active=True)
...
class PostDetailInfoListViewset(mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """
    List:
        文章列表页
    """
    queryset = PostInfo.objects.filter(is_active=True)
    serializer_class = PostDetailInfoSerializer
